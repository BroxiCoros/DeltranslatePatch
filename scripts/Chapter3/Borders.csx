// =====================================================================
// Chapter3/Borders.csx
// =====================================================================
// Version del script NXRUNE_CH3.csx (de NXRUNE, por IRUZZ) adaptada
// para correr DESPUES de que se haya aplicado el mod de traduccion
// deltranslate sobre el data.win de DELTARUNE Capitulo 3.
//
// USO:
//
...
//     carpeta hermana scripts/Chapter3/Borders/ y se descubren via
//     Path.GetDirectoryName(ScriptPath).
//
//   Manual (UndertaleModTool):
//     1. Cargar chapter3_windows/data.win en UndertaleModTool.
//     2. Ejecutar primero scripts/Chapter3/Fix.csx (Scripts -> Run
//        other script).
//     3. Ejecutar este archivo (scripts/Chapter3/Borders.csx) sobre
//        el MISMO data.win, sin guardar entre ambos pasos.
//     4. Guardar el data.win.
//
// QUE CAMBIA RESPECTO AL NXRUNE_CH3.csx ORIGINAL:
//
//   A) gml_Object_obj_darkcontroller_Draw_0  (deltranslate lo REEMPLAZA
//      entero desde Chapter3/CodeEntries):
//        - El patron de busqueda original asume `if (global.is_console)`,
//          `border_options[...]` y `xx + 430` para fullscreenoff/runoff.
//        - Tras pasar deltranslate, ese codigo es:
...
//          `border_options_tr` y `_selectXPos` y conservar las traducciones
//          (claves obj_darkcontroller_slash_Draw_0_gml_93_0..96_0, _112_0,
//          y _Create_0_gml_153_0..2 para los nombres de borde).
//
...
//          archivo (lee BORDER/TYPE del .ini y llama a
//          scr_enable_screen_border en la rama !is_console y en la else).
//        - Eliminamos ese QueueFindReplace para evitar el
//          "no-op find and replace".
//
//   C) gml_GlobalScript_scr_load  (deltranslate lo REEMPLAZA entero):
//        - El parche original de NXRUNE buscaba
//            if (global.is_console)
//                global.tempflag[95] = 1;
//          para quitar el gate de consola.
//        - deltranslate YA QUITO ese gate en su version del archivo
//          (ahora es directamente `global.tempflag[95] = 1;`).
//        - Eliminamos ese QueueFindReplace para evitar el
//          "no-op find and replace".
//
...
// obj_room_ranking_b_Step_0 ni los bloques del obj_initializer2 que
// NXRUNE necesita (deltranslate solo parchea otra parte distinta de
// obj_initializer2_Create_0 / Other_72 que no entra en conflicto).
//
...
// scr_load
//
// NOTA: el QueueFindReplace original sobre gml_GlobalScript_scr_load
// (que quitaba el gate `if (global.is_console)` antes de
// `global.tempflag[95] = 1;`) ha sido eliminado porque deltranslate YA
// reemplaza scr_load entero y en su version el gate ya no existe.
...
importGroup.QueueTrimmedLinesFindReplace("gml_Object_obj_smallface_Draw_64", "cx = camerax() - xoffset_console;", @"var _sfx = 0;
    var _sfy = 0;
    var _sfsx = 1;
    var _sfsy = 1;
